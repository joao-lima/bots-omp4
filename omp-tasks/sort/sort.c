...
#pragma omp taskgroup
  {
    ...
    *(lowdest + lowsize + 1) = *split1;
    ELM *split1_ = split1-1;
#pragma omp task untied depend(inout:low1, low2, lowdest) depend(in:split1_,split2)
    cilkmerge_par(low1, split1_, low2, split2, lowdest);
    
    split1_ = split1+1;
    ELM *split2_ = split2+1;
    ELM *lowdest_ = lowdest+lowsize+2;
#pragma omp task untied depend(inout:split1_, split2_, lowdest_) depend(in:high1, high2)
    cilkmerge_par(split1_, high1, split2_, high2,
                  lowdest_);
  }
...
#pragma omp taskgroup
  {
#pragma omp task untied depend(inout: A, tmpA)
    cilksort_par(A, tmpA, quarter);
#pragma omp task untied depend(inout: B, tmpB)
    cilksort_par(B, tmpB, quarter);
#pragma omp task untied depend(inout: C, tmpC)
    cilksort_par(C, tmpC, quarter);
#pragma omp task untied depend(inout: D, tmpD)
    cilksort_par(D, tmpD, size - 3 * quarter);
    
    ELM *A_ = A + quarter - 1;
    ELM *B_ = B + quarter - 1;
#pragma omp task untied depend(inout:A, B, tmpA) depend(in:A_, B_)
    cilkmerge_par(A, A_, B, B_, tmpA);
    
    ELM *C_ = C + quarter - 1;
    ELM *low_ = low + size - 1;
#pragma omp task untied depend(inout: C, D, tmpC) depend(in:C_, low_)
    cilkmerge_par(C, C_, D, low_, tmpC);
  }
...
#pragma omp task untied depend(inout:array, tmp)
